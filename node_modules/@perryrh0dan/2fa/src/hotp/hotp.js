...
exports.hotpVerify = exports.hotpVerifyDelta = exports.hotpGenerate = exports.digest = void 0;
...
function digest(options) {
    ...
    if (!Buffer.isBuffer(secret)) {
        if (encoding === 'base32') {
            const arrayBuffer = base32_1.base32Decode(secret);
            secretBuffer = utils_1.arrayBufferToBuffer(arrayBuffer);
        }
        else {
    ...
    if (secret_buffer_size && secret.length !== secret_buffer_size) {
        secretBuffer = new Buffer(Array(Math.ceil(secret_buffer_size / secretBuffer.length) + 1).join(secretBuffer.toString('hex')), 'hex').slice(0, secret_buffer_size);
    }
    ...
    const hmac = crypto_1.createHmac(algorithm, secretBuffer);
 ...
 */
function hotpVerifyDelta(options) {
    // unpack options
